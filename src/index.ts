...
const panel1 = new Panel(document.querySelector('.panel'),'X axis','Min','Max', 'duration', 500, 900, 0);
const panel2 = new Panel(document.querySelector('.panel'),'Y axis','Min','Max', 'duration', 500, 1500, 0);
...
addSeriesBtn.element.addEventListener("click", (event) => {
  ...
  const newBtn = new Btn(document.querySelector('.panel'), 'график', plotColor);
  newBtn.element.addEventListener("click", (event) => {
    event.preventDefault()
    chart.data.findSeriesById(seriesId)?.replaceSeriesData([generateArr(series)], 300)
  })
})
...
function generateArr(from: number[]): number[] {
